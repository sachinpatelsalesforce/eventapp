    ...
    return (
      <div>
        ...
        <div className="max-w-md mx-auto px-4 py-12 text-center">
          <p className="text-white/50 mb-4">Register to earn your passport stamps.</p>
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-5">
        ...
        <Card className="p-5">
          <div className="flex justify-between items-center mb-2">
            <h2 className="font-bold text-white">Your Passport</h2>
            <span className="text-sm font-bold text-[#5EB3FF]">{stamps.length}/5 stamps</span>
          </div>
          ...
          {stamps.length === 5 && (
            <p className="text-sm text-green-400 font-medium mt-2">Full passport — well done! 🏆</p>
          )}
        </Card>
        ...
        {loading ? <LoadingSpinner /> : (
          <div className="space-y-3">
            {ALL_STAMPS.map(stampType => {
              ...
              return (
                <Card key={stampType} className={`p-4 ${earned ? 'border-green-500/30 bg-green-500/[0.06]' : ''}`}>
                  <div className="flex items-center gap-4">
                    <div className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl flex-shrink-0 border-2 ${
                      earned ? 'border-green-500/50 bg-green-500/15' : 'border-white/10 bg-white/[0.04] opacity-40'
                    }`}>
                      {STAMP_ICONS[stampType]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`font-semibold text-sm ${earned ? 'text-green-400' : 'text-white'}`}>
                      ...
                      {earned ? (
                        ...
                        <p className="text-xs text-white/30 mt-0.5">Auto-awarded on completion</p>
                      )}
                    </div>
                  </div>
                </Card>

...
const tierStyle: Record<string, string> = {
  summit: 'font-bold text-amber-400',
  platinum: 'font-semibold text-white/80',
  gold: 'font-medium text-yellow-500',
          ...
          <>
            <div className="flex items-center gap-2 flex-wrap mb-4">
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium border ${!activeCapability ? 'bg-[#0070D2] text-white border-[#0070D2]' : 'bg-white/[0.06] text-white/60 border-white/10 hover:border-white/20'}`}
              >
              ...
              {activeCapability && (
                <p className="text-sm text-white/40">
                  ...
                  <strong className="text-white/70">{CAPABILITIES.find(c => c.key === activeCapability)?.label}</strong>
                </p>
              )}
            </div>
              ...
              <div className="min-w-max">
                    ...
                    <button
                      ...
                      className={`w-20 flex-shrink-0 text-center px-1 py-2 text-[11px] font-medium leading-tight transition-colors ${
                        activeCapability === cap.key
                          ? 'text-[#5EB3FF] bg-[#0070D2]/15 rounded-t-lg'
                ...
                {sorted.length === 0 ? (
                  ...
                  sorted.map((partner, i) => (
                    <div
                      key={partner.id}
                      className={`flex items-center ${i % 2 === 0 ? 'bg-white/[0.02]' : 'bg-transparent'} hover:bg-[#0070D2]/10 transition-colors`}
                    >
                      <div className="w-48 flex-shrink-0 pr-3 py-2">
                        <Link href={`/partners/directory/${partner.id}`}>
                          <p className={`text-sm truncate hover:underline ${tierStyle[partner.partner_tier] ?? 'text-white/70'}`}>
                            {partner.si_name}
                          </p>
                      ...
                      {CAPABILITIES.map(cap => (
                        <div
                          key={cap.key}
                          className={`w-20 flex-shrink-0 flex items-center justify-center py-2 ${activeCapability === cap.key ? 'bg-[#0070D2]/10' : ''}`}
                        >
                          {partner.capabilities.includes(cap.key) ? (
                            ...
                            <div className="w-4 h-4 rounded-full bg-white/[0.06] border border-white/10" />
                          )}
                        </div>
                      ))}
                    </div>
            ...
            <div className="flex items-center gap-4 mt-4 text-xs text-white/30">
              ...
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-full bg-white/[0.06] border border-white/10" />
                Not listed
              </span>
            </div>
          </>

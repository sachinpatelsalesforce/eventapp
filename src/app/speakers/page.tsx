            ...
            <Card key={speaker.id} className="p-5">
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="font-bold text-white text-base">{speaker.name}</h2>
                  {(speaker.job_title || speaker.company) && (
                    <p className="text-sm text-[#5EB3FF] font-medium mt-0.5">
                      {[speaker.job_title, speaker.company].filter(Boolean).join(' · ')}
                    </p>
                  )}
                  {speaker.bio && (
                    <p className="text-sm text-white/50 mt-2 line-clamp-3">{speaker.bio}</p>
                  )}
                  {speaker.linkedin_url && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 mt-2 text-xs text-[#5EB3FF] hover:underline font-medium"
                    >
